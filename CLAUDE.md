...
- **Foto Massal**: dari bar aksi seleksi (checkbox) → modal daftar produk terpilih, tiap baris punya input file sendiri, **plus** input "pilih banyak file" yang otomatis menempelkan file ke produk yang SKU-nya muncul di nama file. Pencocokan: ekstensi & akhiran `" (1)"` dibuang dulu (`dasarNamaFile()`), lalu semua karakter non-alfanumerik dihapus (`normalisasiUntukCocok()`), SKU terpanjang dicek duluan supaya `BD-0011` tidak keburu diklaim `BD-001`. Batas **20 produk per batch** (20 produk = 40 unggahan). Unggah berurutan, bukan paralel; kalau satu produk gagal sisanya tetap jalan dan yang gagal dilaporkan per SKU
